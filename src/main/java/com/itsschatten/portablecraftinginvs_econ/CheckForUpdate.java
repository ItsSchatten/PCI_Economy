        ...
        new UpdateNotifications(93136) {
            ...
            public void onUpdateAvailable() {
                ...
                Bukkit.getOnlinePlayers().forEach((admin) -> {
                    if (admin.hasPermission(Permissions.UPDATE.getPermission())) // Check if there are any online players we can send the message to.
                        Utils.tell(admin, UpdateNotifications.getUpdateMessage().replace("{currentVer}", PCI_Econ.getProvidingPlugin(PCI_Econ.class).getDescription().getVersion())
                ...
                Utils.log(UpdateNotifications.getUpdateMessage().replace("{currentVer}",  PCI_Econ.getProvidingPlugin(PCI_Econ.class).getDescription().getVersion())
                        .replace("{newVer}", UpdateNotifications.getLatestVersion())
